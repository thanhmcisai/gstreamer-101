...
int main(int argc, char *argv[])
{
    ...
    gst_bin_add_many(GST_BIN(data.pipeline), data.source, data.audio_convert, data.audio_resample, data.audio_sink, data.video_convert, data.video_sink, NULL);
    if (!gst_element_link_many(data.audio_convert, data.audio_resample, data.audio_sink, NULL))
    {
        g_printerr("Elements could not be audio linked.\n");
    ...
    {
        g_printerr("Elements could not be video linked.\n");
        gst_object_unref(data.pipeline);
        return -1;
    }
