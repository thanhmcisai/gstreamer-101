...
class Player(object):
    def __init__(self):
        ...
        if (
            not self.pipeline or not self.source or not self.audio_convert
                or not self.audio_sink or not self.video_convert
        ...
        if not self.audio_convert.link(self.audio_sink):
            print("ERROR: Could not link 'audio_convert' to 'audio_sink'")
            sys.exit(1)

        if not self.video_convert.link(self.video_sink):
            print("ERROR: Could not link 'video_convert' to 'video_sink'")
            sys.exit(1)
    ...
    def on_pad_added(self, src, new_pad):
        ...
        if new_pad_type.startswith("audio/x-raw"):
            ...
            print("It has type '{0:s}' which is not raw audio or video. Ignoring.".format(
                new_pad_type))
            return
