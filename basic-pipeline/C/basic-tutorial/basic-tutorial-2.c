...
{
    GstElement *pipeline, *source, *sink, *filter_vertigo, *videoconvert;
    GstBus *bus;
    GstMessage *msg;
    ...
    if (!pipeline || !source || !sink || !filter_vertigo || !videoconvert)
    {
    ...
    {
        g_printerr("Elements could not be linked source and filter_vertigo.\n");
        gst_object_unref(pipeline);
        return -1;
    }
    if (gst_element_link(filter_vertigo, videoconvert) != TRUE)
    {
        g_printerr("Elements could not be linked filter_vertigo and videoconvert.\n");
        gst_object_unref(pipeline);
        return -1;
    }
    if (gst_element_link(videoconvert, sink) != TRUE)
    {
        g_printerr("Elements could not be linked videoconvert and sink.\n");
        gst_object_unref(pipeline);
        return -1;
    }
